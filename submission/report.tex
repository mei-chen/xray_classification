...
This report presents a deep learning system for classifying chest X-ray images into three diagnostic categories: \textbf{Normal}, \textbf{Pneumonia}, and \textbf{Tuberculosis}. The system achieves \textbf{92.1\% macro AUC} on a held-out test set of 2,569 images, with particularly strong performance on Pneumonia detection (98.7\% AUC, 99.8\% recall).
...
\textbf{Retraining Results}: Following the ablation study, we retrained the model with the optimized augmentation pipeline (CLAHE at 20\%, reduced rotation to $\pm$10°). The retraining validated our ablation findings:
...
\midrule
Macro AUC & 90.8\% & \textbf{92.1\%} & +1.3\% \\
Accuracy & 77.1\% & \textbf{77.9\%} & +0.8\% \\
TB F1-Score & 75.5\% & \textbf{76.7\%} & +1.2\% \\
Normal Recall & 80.4\% & \textbf{82.2\%} & +1.8\% \\
\bottomrule
\end{tabular}
\caption{Model improvement after retraining with optimized augmentation.}
\end{table}

\noindent The improvements confirm that (1) CLAHE helps with low-contrast TB images, and (2) excessive rotation was harming performance. Training completed in 15 epochs ($\sim$73 minutes) with early stopping.
...
\midrule
Normal & 65.3\% & 82.2\% & 78.5\% & 72.8\% & 85.8\% & 925 \\
Pneumonia & 77.9\% & \textbf{99.8\%} & 91.6\% & 87.5\% & \textbf{98.7\%} & 580 \\
Tuberculosis & \textbf{99.8\%} & 62.2\% & \textbf{99.9\%} & 76.7\% & 91.3\% & 1,064 \\
\midrule
\textbf{Macro Avg} & 81.0\% & 81.4\% & 90.0\% & 79.0\% & \textbf{92.1\%} & 2,569 \\
\textbf{Weighted Avg} & 83.0\% & 77.9\% & -- & 78.4\% & -- & 2,569 \\
\bottomrule
\end{tabular}
...
\begin{itemize}[leftmargin=*]
    \item \textit{Pneumonia}: 99.8\% sensitivity means virtually no missed Pneumonia cases---ideal for screening where false negatives are costly.
    \item \textit{Tuberculosis}: 99.8\% precision and 99.9\% specificity minimize false TB diagnoses (important given treatment implications).
    \item \textit{Normal}: Lower precision (65.3\%) indicates some healthy patients flagged for review---acceptable for screening workflow.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Pneumonia}: Near-perfect recall (99.8\%)---virtually no missed cases, critical for screening.
    \item \textbf{Tuberculosis}: Excellent precision (99.8\%)---very few false positives.
    \item \textbf{Normal}: Some confusion with Pneumonia (16\%), expected given visual similarity.
    \item \textbf{TB$\rightarrow$Normal confusion}: 37.8\% of TB cases misclassified as Normal---improved from 38.3\% with CLAHE augmentation.
\end{itemize}

\subsection{Misclassified Examples Analysis}

To understand model failures qualitatively, we analyzed the 568 misclassified test images (22.1\% error rate).
...
\midrule
TB $\rightarrow$ Normal & 402 & 37.8\% & \textcolor{accent}{\textbf{High}} (missed TB) \\
Normal $\rightarrow$ Pneumonia & 148 & 16.0\% & Low (false alarm) \\
Normal $\rightarrow$ TB & 17 & 1.8\% & Low (false alarm) \\
Pneumonia $\rightarrow$ TB & 1 & 0.2\% & Medium (treatment differs) \\
\bottomrule
\end{tabular}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{TB $\rightarrow$ Normal (402 cases)}: The dominant error pattern. Many misclassified TB images show subtle or early-stage pathology with minimal visible consolidation. The model appears to require more pronounced radiographic findings to confidently predict TB. Note: CLAHE augmentation improved this slightly (from 407 to 402 cases).
...
\midrule
Normal & 0.858 & Good discrimination \\
Pneumonia & 0.987 & Excellent---highly distinctive features \\
Tuberculosis & 0.913 & Very good discrimination \\
\midrule
\textbf{Macro Average} & \textbf{0.921} & Strong overall performance \\
\bottomrule
\end{tabular}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Strong discriminative performance}: 92.1\% macro AUC across three classes
...
\textbf{Limitations}: The main weakness is TB$\rightarrow$Normal confusion (37.8\%), which was slightly improved by CLAHE augmentation but could be further addressed with additional TB training data, class-specific augmentation, or ensemble methods.
